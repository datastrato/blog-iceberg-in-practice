...
# management API, against this part's own Gravitino instance. This is the two
# curls from the README's Quickstart section, wrapped up for convenience -- run
# it by hand once per fresh `docker compose up`, after catalog-dynamic is healthy.
#
# `credential-providers` is set on the catalog from the start (rather than added
# later with a follow-up PUT): "type": "RELATIONAL" and
...
curl -sS -X POST "$BASE/metalakes" \
  ...
  -d '{"name": "demo_metalake", "comment": "iceberg-in-practice part-1 rig"}'
echo
